...
{
	static reg_type& value(const address_type addr)
	{
		return *reinterpret_cast<reg_type*>(addr);
	}
	
	static bits_type& bits(const address_type addr)
	{
...
int main()
{
	...
	reinterpret_cast<volatile reg_map_c*>(gpiob)->value = 0xFF;
	reg_map_dynamic<std::uint32_t,
									std::uint32_t,
	...
	{
		/*Toggle Pin 12*/
		//reinterpret_cast<volatile bit32_type*>(gpiob)->b12 ^=1U;
		//reinterpret_cast<volatile reg_map_c*>(gpiob)->bits.b12  ^=1U;
		reg_map_dynamic<std::uint32_t,std::uint32_t,bit32_type>::bits(gpiob).b12 ^= 1U;
